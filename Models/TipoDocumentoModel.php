<?php 
class TrabajadoresModel extends Mysql
{
	private $int;
	...
	public function insertTrabajador(string $identificacion, string $nombre, string $apellido, int $telefono, string $email, string $password, int $tipoid, string $nit, string $nomFiscal, string $dirFiscal){
	...
	{
		$sql = "SELECT w.idWorkers,w.name,w.lastName,w.phone,w.status,dt.desdcriptionDocumentType, w.identification
		FROM workers w inner join documenttypes dt where w.documentTypesId  = dt.idDocumentTypes "; 
		$request = $this->select_all($sql);
		return $request;
		var_dump($request);
	}

	public function selectTrabajador(int $idpersona){
		...
		$sql = "SELECT idpersona,identificacion,nombres,apellidos,telefono,email_user,nit,nombrefiscal,direccionfiscal,status, DATE_FORMAT(datecreated, '%d-%m-%Y') as fechaRegistro 
				FROM persona
				WHERE idpersona = $this->intIdUsuario and rolid = ".RWorkers;
		$request = $this->select($sql);
		return $request;
	}

	public function updateWorker(int $idUsuario, string $identificacion, string $nombre, string $apellido, int $telefono, string $email, string $password, string $nit, string $nomFiscal, string $dirFiscal){
	...
	public function deleteWorker(int $intIdpersona)
	{
