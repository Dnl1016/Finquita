...
class TiposDocumentos extends Controllers{
	...
	public function Trabajadores()
	{
		...
		$data['page_title'] = "Trabajadores <small>La finquita</small>";
		$data['page_name'] = "trabajadores";
		$data['page_functions_js'] = "functions_trabajadores.js";
	...
	public function getTiposDocumentos()
	{
		if($_SESSION['permisosMod']['r']){
			$arrData = $this->model->selectgetTiposDocumentos();
			for ($i=0; $i < count($arrData); $i++) {
				$btnView = '';
				$btnEdit = '';
				$btnDelete = '';
				if($_SESSION['permisosMod']['r']){
					$btnView = '<button class="btn btn-info btn-sm" onClick="fntViewInfo('.$arrData[$i]['idDocumentTypes'].')" title="Ver Tipo de documentos"><i class="far fa-eye"></i></button>';
				}
				if($_SESSION['permisosMod']['u']){
					$btnEdit = '<button class="btn btn-primary  btn-sm" onClick="fntEditInfo(this,'.$arrData[$i]['idDocumentTypes'].')" title="Editar Tipo de documentos"><i class="fas fa-pencil-alt"></i></button>';
				}
				if($_SESSION['permisosMod']['d']){	
					$btnDelete = '<button class="btn btn-danger btn-sm" onClick="fntDelInfo('.$arrData[$i]['idDocumentTypes'].')" title="Eliminar Tipo de documentos"><i class="far fa-trash-alt"></i></button>';
				}
